"""Reliability watchdog — keeps the ingest worker + model server alive and the GPU from
saturating. Automates the exact recoveries done by hand this session:
...
PY = os.path.join(AIWORKER, ".venv", "Scripts", "python.exe")
...
def launch(args, name):
    ...
    subprocess.Popen([PY] + args, cwd=AIWORKER, stdout=out, stderr=subprocess.STDOUT,
    ...
    while True:
        try:
            if proc_count(WORKER_PAT) == 0:
                launch(["-m", "arq", "worker.main.WorkerSettings"], "arqworker")
            if proc_count(SERVER_PAT) == 0:
                launch(["-m", "uvicorn", "worker.server:app", "--host", "127.0.0.1", "--port", "8100"], "modelserver")
                time.sleep(15)
            ...
            log(f"ok vram={vram}MB server_up={resp} worker={proc_count(WORKER_PAT)} ingest={ingest_active()}")
